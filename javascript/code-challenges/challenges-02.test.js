...
const raisedToTheThird = (arr) => {
  ...
  arr.forEach(item => arrayOfNumbers.push(Math.pow(item, 3)));
...
const addOne = (arr) => {
  const map1 = arr.map(item => item+1);
  return map1;
};
...
const forEachTwoToThe = (arr) => {
  const arrayOfTwoToThe = [];
  arr.forEach(item => {
    arrayOfTwoToThe.push(Math.pow(2, item));
  });
  return arrayOfTwoToThe;
};
